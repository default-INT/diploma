import addIcon from './icon/add.svg';
import bellIcon from './icon/bell.svg';
import builderIcon from './icon/builder.svg';
import calendarIcon from './icon/calendar.svg';
import deleteIcon from './icon/delete.svg';
import dollarIcon from './icon/dollar.svg';
import editIcon from './icon/edit.svg';
import employeeIcon from './icon/employee.svg';
import palletIcon from './icon/pallet.svg';
import palletWhiteIcon from './icon/pallet-white.svg';
import palletMainIcon from './icon/pallet-icon.svg';
import popularityIcon from './icon/popularity.svg';
import questionIcon from './icon/question.svg';
import questionWhiteIcon from './icon/question-white.svg';
import salaryIcon from './icon/salary.svg';
import leftArrowBlackIcon from './icon/left-arrow-black.svg';
import leftArrowWhiteIcon from './icon/left-arrow-white.svg';
import rightArrowBlackIcon from './icon/right-arrow-black.svg';
import rightArrowWhiteIcon from './icon/right-arrow-white.svg';
import doneBlueIcon from './icon/done-blue.svg';
import questionOrangeIcon from './icon/question-orange.svg';
import questionRedIcon from './icon/question-red.svg';

import navBuilderWhiteIcon from './icon/nav/builder-white.svg';
import navCalendarCircuitIcon from './icon/nav/calendar-circuit.svg';
import navCalendarCircuitWhiteIcon from './icon/nav/calendar-circuit-white.svg';
import navCalendarSolidWhiteIcon from './icon/nav/calendar-solid-white.svg';
import navEmployeeWhiteIcon from './icon/nav/employee-white.svg';
import navSuitcaseIcon from './icon/nav/suitcase.svg';
import navSuitcaseWhiteIcon from './icon/nav/suitcase-white.svg';
import navChart from "./icon/nav/chart.svg";
import navExport from "./icon/nav/export.svg";
import navStore from "./icon/nav/store.svg";


export {
    addIcon, bellIcon, builderIcon, calendarIcon, deleteIcon, dollarIcon,
    editIcon, employeeIcon, palletIcon, palletWhiteIcon, palletMainIcon,
    popularityIcon, questionIcon, salaryIcon, questionWhiteIcon, leftArrowWhiteIcon,
    leftArrowBlackIcon, rightArrowBlackIcon, rightArrowWhiteIcon, doneBlueIcon, questionOrangeIcon,
    questionRedIcon,

    navBuilderWhiteIcon, navCalendarCircuitIcon, navCalendarCircuitWhiteIcon,
    navCalendarSolidWhiteIcon, navEmployeeWhiteIcon, navSuitcaseIcon, navSuitcaseWhiteIcon,
    navChart, navExport, navStore
}